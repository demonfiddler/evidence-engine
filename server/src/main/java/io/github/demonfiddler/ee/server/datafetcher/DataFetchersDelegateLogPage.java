 ...
 * fetching fields for the LogPage GraphQL type, as defined in the provided GraphQL schema. Please read the
...
public interface DataFetchersDelegateLogPage {
	 ...
	 */
	Map<LogPage, List<Log>> content(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		List<LogPage> keys);
	 ...
	 */
	default List<LogPage> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<LogPage> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegateLogPage implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * @return
	 */
	default List<LogPage> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
