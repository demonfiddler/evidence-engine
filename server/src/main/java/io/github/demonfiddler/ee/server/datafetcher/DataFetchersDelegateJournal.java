 ...
 * fetching fields for the Journal GraphQL type, as defined in the provided GraphQL schema. Please read the
...
public interface DataFetchersDelegateJournal {
	 ...
	 * This method loads the data for Journal.status. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 ...
	 * the status attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Journal,
	 ...
	 * params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:title, type:String,
	 * params:[]},Field{name:abbreviation, type:String, params:[]},Field{name:url, type:URL, params:[]},Field{name:issn,
	 * type:ISSN, params:[]},Field{name:publisher, type:Publisher, params:[]},Field{name:notes, type:String,
	 ...
	 */
	Object status(DataFetchingEnvironment dataFetchingEnvironment, Journal origin, FormatKind format);
	 ...
	 * This method loads the data for Journal.log. It may return whatever is accepted by the Spring Controller, that is:
	 * <ul>
	 ...
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the log attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Journal, fields:{Field{name:id,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object log(DataFetchingEnvironment dataFetchingEnvironment, DataLoader<Long, LogPage> dataLoader, Journal origin,
		LogQueryFilter filter, PageableInput pageSort);

	/**
	 * This methods loads the data for ${dataFetcher.graphQLType}.publisher. It is generated as the
	 * <code>generateBatchMappingDataFetchers</code> plugin parameter is true. <br/>
	 * @param batchLoaderEnvironment The environement for this batch loaded. You can extract the GraphQLContext from
	 * this parameter.
	 * @param graphQLContext
	 ...
	 */
	default List<Journal> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<Journal> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegateJournal implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * @return
	 */
	default List<Journal> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
