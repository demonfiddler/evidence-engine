 ...
 * fetching fields for the Claim GraphQL type, as defined in the provided GraphQL schema. Please read the
...
public interface DataFetchersDelegateClaim {

	/**
	 * Description for the status field: <br/>
	 * The entity status. <br/>
	 * This method loads the data for Claim.status. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 ...
	 * the status attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Claim,
	 ...
	 * params:[filter:TopicRefQueryFilter,pageSort:PageableInput]},Field{name:text, type:String,
	 * params:[]},Field{name:date, type:Date, params:[]},Field{name:notes, type:String, params:[]}}, implements
	 * IBaseEntity,ITrackedEntity,ITopicalEntity, comments=empty}. It depends on your data modle, but it typically
	 ...
	 */
	Object status(DataFetchingEnvironment dataFetchingEnvironment, Claim origin, FormatKind format);
	 ...
	 */
	Map<Claim, User> createdByUser(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		List<Claim> keys);
	 ...
	 * Please look at the spring-graphql annotation for a documentation on how to return the proper values
	 */
	Map<Claim, User> updatedByUser(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		List<Claim> keys);

	/**
	 * Description for the log field: <br/>
	 * Log of transactions involving the record. <br/>
	 * This method loads the data for Claim.log. It may return whatever is accepted by the Spring Controller, that is:
	 * <ul>
	 ...
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the log attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Claim, fields:{Field{name:id,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object log(DataFetchingEnvironment dataFetchingEnvironment, DataLoader<Long, LogPage> dataLoader, Claim origin,
		LogQueryFilter filter, PageableInput pageSort);

	/**
	 * Description for the topicRefs field: <br/>
	 * The topic(s) associated with the claim. <br/>
	 * This method loads the data for Claim.topicRefs. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 ...
	 * <A HREF= "https://github.com/graphql-java/java-dataloader">https://github.com/graphql-java/java-dataloader</A>
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the topicRefs attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Claim,
	 * fields:{Field{name:id, type:ID!, params:[]},Field{name:status, type:String,
	 * params:[format:FormatKind]},Field{name:created, type:DateTime, params:[]},Field{name:createdByUser, type:User,
	 * params:[]},Field{name:updated, type:DateTime, params:[]},Field{name:updatedByUser, type:User,
	 * params:[]},Field{name:log, type:LogPage!,
	 ...
	 */
	default List<Claim> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<Claim> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegateClaim implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * returned list.</LI>
	 * </UL>
	 * @param keys A list of IDs, for which the matching objects must be returned
	 * @param environment The Data Loader environment
	 * @return
	 */
	default List<Claim> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
