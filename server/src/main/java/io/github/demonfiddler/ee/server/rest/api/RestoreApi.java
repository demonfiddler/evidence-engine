...
@Validated
@Tag(name = "restore", description = "the restore API")
public interface RestoreApi {
    ...
    String PATH_RESTORE = "/restore";

    /**
     * POST /restore : Restore the database Restores the database from a zipped collection of CSV files
     * @param kind Whether to include static lookup table data or just the application data (required)
     * @param file (optional)
     * @return Plain HTML response to any operation (status code 200) or Bad request (e.g., invalid combination of query
     * parameters) (status code 400) or Internal server error (status code 500)
     */
    @Operation(operationId = "restore", summary = "Restore the database",
        description = "Restores the database from a zipped collection of CSV files",
        responses = {
            @ApiResponse(responseCode = "200", description = "Plain HTML response to any operation",
    ...
    @RequestMapping(method = RequestMethod.POST, value = RestoreApi.PATH_RESTORE, produces = { "text/html" },
        consumes = { "multipart/form-data" })
    default ResponseEntity<String> restore(
        @NotNull @Parameter(name = "kind",
