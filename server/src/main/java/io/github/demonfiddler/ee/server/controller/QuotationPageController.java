 ...
 */
@Controller
@SchemaMapping(typeName = "QuotationPage")
@SuppressWarnings("unused")
public class QuotationPageController {

	@Autowired
	protected DataFetchersDelegateQuotationPage dataFetchersDelegateQuotationPage;

	@Autowired
	protected GraphqlServerUtils graphqlServerUtils;

	public QuotationPageController(BatchLoaderRegistry registry) {
		...
		registry.forTypePair(Long.class, QuotationPage.class).registerMappedBatchLoader((keysSet, env) -> {
			List<Long> keys = new ArrayList<>(keysSet.size());
			keys.addAll(keysSet);
			return Mono.fromCallable(() -> {
				Map<Long, QuotationPage> map = new HashMap<>();
				// Values are returned in the same order as the keys list
				List<QuotationPage> values = this.dataFetchersDelegateQuotationPage.batchLoader(keys, env);
	...
	public Flux<Quotation> content(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		...
		return this.dataFetchersDelegateQuotationPage.content(batchLoaderEnvironment, graphQLContext, keys);
	}
