 ...
 */
@Controller
@SchemaMapping(typeName = "ClaimPage")
public class ClaimPageController {

	@Autowired
	protected DataFetchersDelegateClaimPage dataFetchersDelegateClaimPage;
	...
	public ClaimPageController(BatchLoaderRegistry registry) {
		...
		registry.forTypePair(Long.class, ClaimPage.class).registerMappedBatchLoader((keysSet, env) -> {
			...
			return Mono.fromCallable(() -> {
				Map<Long, ClaimPage> map = new HashMap<>();
				// Values are returned in the same order as the keys list
				List<ClaimPage> values = this.dataFetchersDelegateClaimPage.batchLoader(keys, env);
	...
	public Map<ClaimPage, List<Claim>> content(BatchLoaderEnvironment batchLoaderEnvironment,
		...
		return this.dataFetchersDelegateClaimPage.content(batchLoaderEnvironment, graphQLContext, keys);
	}
