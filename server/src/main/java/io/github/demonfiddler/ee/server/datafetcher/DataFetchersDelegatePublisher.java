 ...
 * fetching fields for the Publisher GraphQL type, as defined in the provided GraphQL schema. Please read the
...
public interface DataFetchersDelegatePublisher {

	/**
	 * Description for the status field: <br/>
	 * The entity status. <br/>
	 * This method loads the data for Publisher.status. It may return whatever is accepted by the Spring Controller,
	 * that is:
	 * <ul>
	 ...
	 * the status attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Publisher,
	 ...
	 * params:[]},Field{name:log, type:LogPage!, params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:name,
	 * type:String, params:[]},Field{name:location, type:String, params:[]},Field{name:country, type:String,
	 * params:[format:CountryFormatKind]},Field{name:url, type:URL, params:[]},Field{name:journalCount, type:Int,
	 * params:[]}}, implements IBaseEntity,ITrackedEntity, comments ""}. It depends on your data modle, but it typically
	 ...
	 */
	Object status(DataFetchingEnvironment dataFetchingEnvironment, Publisher origin, FormatKind format);
	 ...
	 */
	Map<Publisher, User> createdByUser(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		List<Publisher> keys);
	 ...
	 * Please look at the spring-graphql annotation for a documentation on how to return the proper values
	 */
	Map<Publisher, User> updatedByUser(BatchLoaderEnvironment batchLoaderEnvironment, GraphQLContext graphQLContext,
		List<Publisher> keys);

	/**
	 * Description for the log field: <br/>
	 * Log of transactions involving the record. <br/>
	 * This method loads the data for Publisher.log. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 ...
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the log attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Publisher,
	 * fields:{Field{name:id, type:ID!, params:[]},Field{name:status, type:String,
	 * params:[format:FormatKind]},Field{name:created, type:DateTime, params:[]},Field{name:createdByUser, type:User,
	 * params:[]},Field{name:updated, type:DateTime, params:[]},Field{name:updatedByUser, type:User,
	 * params:[]},Field{name:log, type:LogPage!, params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:name,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object log(DataFetchingEnvironment dataFetchingEnvironment, DataLoader<Long, LogPage> dataLoader, Publisher origin,
		LogQueryFilter filter, PageableInput pageSort);

	/**
	 * Description for the country field: <br/>
	 * The publisher country. <br/>
	 * This method loads the data for Publisher.country. It may return whatever is accepted by the Spring Controller,
	 * that is:
	 * <ul>
	 * <li>A resolved value of any type (typically, a String)</li>
	 * <li>Mono and Flux for asynchronous value(s). Supported for controller methods and for any DataFetcher as
	 ...
	 * GraphQL context for this DataFetcher
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the country attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Publisher,
	 * fields:{Field{name:id, type:ID!, params:[]},Field{name:status, type:String,
	 * params:[format:FormatKind]},Field{name:created, type:DateTime, params:[]},Field{name:createdByUser, type:User,
	 * params:[]},Field{name:updated, type:DateTime, params:[]},Field{name:updatedByUser, type:User,
	 * params:[]},Field{name:log, type:LogPage!, params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:name,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object country(DataFetchingEnvironment dataFetchingEnvironment, Publisher origin, CountryFormatKind format);
	 ...
	 */
	default List<Publisher> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<Publisher> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegatePublisher implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * @return
	 */
	default List<Publisher> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
