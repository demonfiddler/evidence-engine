...
public interface DataFetchersDelegateUser {
	 ...
	 * This method loads the data for User.status. It may return whatever is accepted by the Spring Controller, that is:
	 * <ul>
	 ...
	 * the status attribute of the <I>origin</I>, which is an instance of {ObjectType {name:User, fields:{Field{name:id,
	 ...
	 */
	Object status(DataFetchingEnvironment dataFetchingEnvironment, User origin, FormatKind format);
	 ...
	 * This method loads the data for User.log. It may return whatever is accepted by the Spring Controller, that is:
	 * <ul>
	 ...
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the log attribute of the <I>origin</I>, which is an instance of {ObjectType {name:User, fields:{Field{name:id,
	 * type:ID!, params:[]},Field{name:status, type:String, params:[format:FormatKind]},Field{name:created,
	 * type:DateTime, params:[]},Field{name:createdByUser, type:User, params:[]},Field{name:updated, type:DateTime,
	 * params:[]},Field{name:updatedByUser, type:User, params:[]},Field{name:log, type:LogPage!,
	 * params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:login, type:String,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object log(DataFetchingEnvironment dataFetchingEnvironment, DataLoader<Long, LogPage> dataLoader, User origin,
	 ...
	 * This method loads the data for User.permissions. It may return whatever is accepted by the Spring Controller,
	 * that is:
	 * <ul>
	 ...
	 */
	default List<User> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<User> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegateUser implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * @return
	 */
	default List<User> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
