...
public interface DataFetchersDelegateTopic {
	 ...
	 * This method loads the data for Topic.status. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 ...
	 * the status attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Topic,
	 ...
	 * params:[filter:LogQueryFilter,pageSort:PageableInput]},Field{name:label, type:String,
	 * params:[]},Field{name:description, type:String, params:[]},Field{name:parent, type:Topic,
	 * params:[]},Field{name:children, type:[Topic!], params:[]},Field{name:entities, type:[ITopicalEntity!],
	 * params:[entityKind:EntityKind!,filter:TopicalEntityQueryFilter,pageSort:PageableInput]}}, implements
	 ...
	 */
	Object status(DataFetchingEnvironment dataFetchingEnvironment, Topic origin, FormatKind format);
	 ...
	 * This method loads the data for Topic.log. It may return whatever is accepted by the Spring Controller, that is:
	 * <ul>
	 ...
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the log attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Topic, fields:{Field{name:id,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object log(DataFetchingEnvironment dataFetchingEnvironment, DataLoader<Long, LogPage> dataLoader, Topic origin,
	...
	/**
	 * Description for the entities field: <br/>
	 * Referenced entities. <br/>
	 * This method loads the data for Topic.entities. It may return whatever is accepted by the Spring Controller, that
	 * is:
	 * <ul>
	 * <li>A resolved value of any type (typically, a List<io.github.demonfiddler.ee.server.ITopicalEntity>)</li>
	 * <li>Mono and Flux for asynchronous value(s). Supported for controller methods and for any DataFetcher as
	 * described in Reactive DataFetcher. This would typically be a
	 * Mono&lt;List<io.github.demonfiddler.ee.server.ITopicalEntity>&gt; or a
	 * Flux&lt;List<io.github.demonfiddler.ee.server.ITopicalEntity>&gt;</li>
	 * <li>Kotlin coroutine and Flow are adapted to Mono and Flux</li>
	 * <li>java.util.concurrent.Callable to have the value(s) produced asynchronously. For this to work,
	 * AnnotatedControllerConfigurer must be configured with an Executor. This would typically by a
	 * Callable&lt;List<io.github.demonfiddler.ee.server.ITopicalEntity>&gt;</li>
	 * </ul>
	 * As a complement to the spring-graphql documentation, you may also return:
	 * <ul>
	 * <li>A CompletableFuture<?>, for instance
	 * CompletableFuture<List<io.github.demonfiddler.ee.server.ITopicalEntity>>. This allows to use
	 * <A HREF="https://github.com/graphql-java/java-dataloader">graphql-java java-dataloader</A> to highly optimize the
	 * number of requests to the server. The principle is this one: The data loader collects all the data to load, avoid
	 * to load several times the same data, and allows parallel execution of the queries, if multiple queries are to be
	 * run.</li>
	 ...
	 * GraphQL context for this DataFetcher
	 * @param origin The object from which the field is fetch. In other word: the aim of this data fetcher is to fetch
	 * the entities attribute of the <I>origin</I>, which is an instance of {ObjectType {name:Topic,
	 * fields:{Field{name:id, type:ID!, params:[]},Field{name:status, type:String,
	 * params:[format:FormatKind]},Field{name:created, type:DateTime, params:[]},Field{name:createdByUser, type:User,
	 * params:[]},Field{name:updated, type:DateTime, params:[]},Field{name:updatedByUser, type:User,
	 * params:[]},Field{name:log, type:LogPage!,
	 ...
	 * IBaseEntity,ITrackedEntity, comments ""}. It depends on your data modle, but it typically contains the id to use
	 * in the query.
	 * @param entityKind The input parameter sent in the query by the GraphQL consumer, as defined in the GraphQL
	 * schema.
	 * @param filter The input parameter sent in the query by the GraphQL consumer, as defined in the GraphQL schema.
	 * @param pageSort The input parameter sent in the query by the GraphQL consumer, as defined in the GraphQL schema.
	 * @throws NoSuchElementException This method may return a {@link NoSuchElementException} exception. In this case,
	 ...
	 * will take care of the {@link NoSuchElementException} exception.
	 */
	Object entities(DataFetchingEnvironment dataFetchingEnvironment, Topic origin, EntityKind entityKind,
	 ...
	 */
	default List<Topic> batchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		List<Topic> ret = unorderedReturnBatchLoader(keys, environment);
		if (ret == null) {
			throw new NullPointerException(
				"Either batchLoader or unorderedReturnBatchLoader must be overriden in DataFetchersDelegateTopic implementation. And unorderedReturnBatchLoader must return a list."); //$NON-NLS-1$
		}
	 ...
	 * @return
	 */
	default List<Topic> unorderedReturnBatchLoader(List<Long> keys, BatchLoaderEnvironment environment) {
		return null;
	}
