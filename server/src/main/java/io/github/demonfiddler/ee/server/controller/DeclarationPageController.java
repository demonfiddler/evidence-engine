 ...
 */
@Controller
@SchemaMapping(typeName = "DeclarationPage")
public class DeclarationPageController {

	@Autowired
	protected DataFetchersDelegateDeclarationPage dataFetchersDelegateDeclarationPage;

	@Autowired
	protected GraphqlServerUtils graphqlServerUtils;

	public DeclarationPageController(BatchLoaderRegistry registry) {
		...
		registry.forTypePair(Long.class, DeclarationPage.class).registerMappedBatchLoader((keysSet, env) -> {
			List<Long> keys = new ArrayList<>(keysSet.size());
			keys.addAll(keysSet);
			return Mono.fromCallable(() -> {
				Map<Long, DeclarationPage> map = new HashMap<>();
				// Values are returned in the same order as the keys list
				List<DeclarationPage> values = this.dataFetchersDelegateDeclarationPage.batchLoader(keys, env);
	...
	/**
	 * Loads the data for DeclarationPage.content. It is generated as the
	...
	public Map<DeclarationPage, List<Declaration>> content(BatchLoaderEnvironment batchLoaderEnvironment,
		...
		return this.dataFetchersDelegateDeclarationPage.content(batchLoaderEnvironment, graphQLContext, keys);
	}
