...
class LinkableEntityTests extends AbstractLinkTests {
    ...
    // LinkableEntityTests needs to do paged, sorted and filtered on: topicId, recursive,
    // fromEntityKind/fromEntityId, status, text

    private static final Logger LOGGER = LoggerFactory.getLogger(LinkableEntityTests.class);
    ...
    private static final int[][] TOPIC_ENTITY_LINK_INDEXES = { //
    ...
    private static final int[][] TOPIC_ENTITY_LINK_RECURSIVE_INDEXES = { //
        { 0 }, // Updated parent label
        {}, // Updated child label
    ...
    private static final int[][] MASTER_ENTITY_LINK_INDEXES = { //
        /*{}*/{ 0 }, // Updated test title
        { 1, 2, 7, 8 }, // ENTITY ONE
        { 2, 3, 6, 7 }, // Entity two
    ...
    private void readEntities(EntityKind entityKind, Long topicId, Boolean recursive, EntityKind masterEntityKind,
        ...
        LinkableEntityQueryFilter filter = topicId == null && recursive == null && masterEntityKind == null
            && masterEntityId == null && status == null && text == null //
                ? null //
                : LinkableEntityQueryFilter.builder() //
                    .withTopicId(topicId) //
                    .withRecursive(recursive) //
                    .withFromEntityKind(masterEntityKind) //
                    .withFromEntityId(masterEntityId) //
                    .withStatus(status) //
                    .withText(text) //
                    .build();
    ...
    private void checkPage(AbstractPage<?> actual, boolean status, boolean text, Long topicId, boolean recursive,
        ...
        if (topicId == null) {
            ...
            topicLinkedIndexes = //
                recursive //
                    ? TOPIC_ENTITY_LINK_RECURSIVE_INDEXES[topicIndex] //
                    : TOPIC_ENTITY_LINK_INDEXES[topicIndex];
        }
        int[] masterEntityLinkedIndexes = //
            (masterEntityKind == null || masterEntityId == null) //
                ? ALL_ENTITY_INDEXES //
                : MASTER_ENTITY_LINK_INDEXES[getEntityIndex(masterEntityKind, masterEntityId)];
        int[] allIndexes = //
        ...
        if (sorted) {
            int[] sortedIndexes = ignoreCase || CASE_INSENSITIVE ? SORTED_IGNORE_CASE_INDEXES : SORTED_INDEXES;
